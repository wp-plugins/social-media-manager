		...
		if (is_array($value))
        {
	        ...
	        for ($x=0;$x<count($value);$x++)
	        {
		        ...
		        $results[] = array("id"=>$storyclass->id,"link"=>$storyclass->link,"submit_date"=>strftime("%b %d %Y %H:%M:%S",$storyclass->submit_date),"diggs"=>$storyclass->diggs,"comments"=>$storyclass->comments,
		        			...
		        			"topic_name"=>$storyclass->topic->name,"topic_short_name"=>$storyclass->topic->short_name,"container_name"=>$storyclass->container->name,"container_short_name"=>$storyclass->container->short_name);
	    	}  
		} else {
